...
?>
<form class="quiz-buttons quiz-question-nav-buttons" method="post">

	<?php if ( $user->has( 'quiz-status', array( 'started' ), $quiz->id ) ): ?>
		<button class="button-prev-question"><?php esc_html_e( 'Previous', 'elearningwp' ); ?></button>
		<button class="button-next-question"><?php esc_html_e( 'Next', 'elearningwp' ); ?></button>
		<?php if ( !$user->has_checked_answer( $question->id, $quiz->id, $course->id ) ): ?>
			<button
				...
				<?php esc_html_e( 'Check', 'elearningwp' ); ?>
			</button>
			<button class="button-hint" data-security="<?php echo esc_attr( wp_create_nonce( 'get-question-hint-' . $user->id . '-' . $course->id . '-' . $quiz->id ) ); ?>"><?php esc_html_e( 'Hint', 'elearningwp' ); ?></button>
		<?php endif; ?>
	<?php endif; ?>
	<?php if ( $user->has( 'quiz-status', 'completed', $quiz->id ) && $remain = $user->can( 'retake-quiz', $quiz->id ) ): ?>
		<button
			...
			<?php echo esc_html( sprintf( '%s (+%d)', __( 'Retake', 'elearningwp' ), $remain ) ); ?>
		</button>
	<?php endif; ?>

	<?php if ( $user->can_do_quiz( $quiz->id, $course->id ) ): ?>
		<button
			...
			<?php esc_html_e( 'Start Quiz', 'elearningwp' ); ?>
		</button>
	<?php endif; ?>
	...
	<?php if ( in_array( $status, array( 'started' ) ) ): ?>
		<button
			...
			<?php esc_html_e( 'Finish Quiz', 'elearningwp' ); ?>
		</button>
	<?php endif; ?>
