...
<div class="filter_courses">
	...
	<ul class="filter">
		<li class="active" rel="<?php echo esc_url( home_url() ) . '/' . $courses_page->post_name; ?>/?cols=<?php echo $instance['columns']; ?>&number=<?php echo $limit; ?>">
			<a href=""><?php echo esc_html__( 'All', 'elearningwp' ); ?></a></li>
		<?php
		if ( ! empty( $query ) ) {
			foreach ( $query as $key => $value ) {
				?>
				<li rel="<?php echo esc_url( get_category_link( $value->term_id ) ); ?>/?cols=<?php echo $instance['columns']; ?>&number=<?php echo $limit; ?>">
					<a href=""><?php echo $value->name; ?></a></li>
				<?php
			}
		}
		?>
